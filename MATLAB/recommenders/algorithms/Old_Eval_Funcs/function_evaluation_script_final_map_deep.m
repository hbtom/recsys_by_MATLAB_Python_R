function output = function_evaluation_script_final_map_deep(fature_name,fold_no,cut_off,flag_rec)
...
elseif ispc
 ...
 load(fullfile(outAddr,[fature_name '_fld_' num2str(fold_no) 'of5_NEW.mat']));
...
fold_no
 output = evaluate_urms_custom_map_avf(urmTest_New,urmPred1_str,urmPred_base1_str,urmPred_base2_str,cut_off,flag_rec) ;
 save(fullfile(outAddr,'Eval',['Eval_res_' 'RNADOM' '_fld_' ...
         num2str(fold_no) 'of5_cutoff_' num2str(cut_off) '_MAP_rec' num2str(flag_rec) '.mat']),'output','fold_no','cut_off','-v7.3');
