function output = function_evaluation_script_Borda(feature_name1,feature_name2,fold_no,cut_off,flag_rec,metric1,metric2,metric3,metric4,baselineFeature)
...
%         feature_name1 = [baselineFeature '_fld_' num2str(fold_no) 'of5_NEW'];
%     end
%     save(fullfile(outAddr,'Eval',['Eval_res_' feature_name1 '_cutoff_' num2str(cut_off) '_MAP_rec' num2str(flag_rec) '.mat']),'output','fold_no','cut_off','-v7.3');
% end
if strcmp(metric2,'MRR')
    
    output = evaluate_urms_custom_mrr_Borda(urmTest_New,urmPred1_str,urmPred2_str,cut_off,flag_rec,baselineFeature);
    if strcmp(baselineFeature,'Random')
        feature_name1 = [baselineFeature '_fld_' num2str(fold_no) 'of5_NEW'];
    end
    
%     save(fullfile(outAddr,'Eval',['Eval_res_Borda' feature_name1 '_' feature_name2 '_cutoff_' num2str(cut_off) '_MRR_rec' num2str(flag_rec) '.mat']),'output','fold_no','cut_off','-v7.3');
end
...
%         feature_name1 = [baselineFeature '_fld_' num2str(fold_no) 'of5_NEW'];
%     end
%     save(fullfile(outAddr,'Eval',['Eval_res_' feature_name1 '_cutoff_' num2str(cut_off) '_Precision_rec' num2str(flag_rec) '.mat']),'output','fold_no','cut_off','-v7.3');
% end
...
%         feature_name1 = [baselineFeature '_fld_' num2str(fold_no) 'of5_NEW'];
%     end
%     save(fullfile(outAddr,'Eval',['Eval_res_' feature_name1 '_cutoff_' num2str(cut_off) '_Recall_rec' num2str(flag_rec) '.mat']),'output','fold_no','cut_off','-v7.3');
% end
