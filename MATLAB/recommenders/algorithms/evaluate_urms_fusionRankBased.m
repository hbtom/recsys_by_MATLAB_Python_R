function output = evaluate_urms_fusionRankBased(urmTest, urmPred1, urmPred2)
%
...
%         [rp_u1,ranked_list1] = sort(urmPred1(u,:),'descend');
%         [rp_u2,ranked_list2] = sort(urmPred2(u,:),'descend');
        
        
        Nrl1 = max(sigma_u_1);
        Nrl2 = max(sigma_u_2);
        ranked_list3 = 0.5*(Nrl1-sigma_u_1+1)/Nrl1 + 0.5*(Nrl2-sigma_u_2+1)/Nrl2;
        ...
        [rp_u,ranked_list] = sort(ranked_list3,'descend');
             ...
             if mod(p,25000)==1
              ...
              PR11_u(p) = precision_at_k(ranked_list1,pos_items_ids,1); 
              PR12_u(p) = precision_at_k(ranked_list2,pos_items_ids,1); 

              PR1ag_u(p) = precision_at_k(ranked_list,pos_items_ids,1); 
...
%               PR5_u(p) = precision_at_k(ranked_list,pos_items_ids,5) ;
              PR101_u(p) = precision_at_k(ranked_list1,pos_items_ids,10) ;
              PR102_u(p) = precision_at_k(ranked_list2,pos_items_ids,10) ;
              PR10ag_u(p) = precision_at_k(ranked_list,pos_items_ids,10) ;

              
              PR1001_u(p) = precision_at_k(ranked_list1,pos_items_ids,100) ;
              PR1002_u(p) = precision_at_k(ranked_list2,pos_items_ids,100) ;
              PR100ag_u(p) = precision_at_k(ranked_list,pos_items_ids,100) ;
...
[mean(PR11_u) mean(PR12_u) mean(PR1ag_u)]
[mean(PR101_u) mean(PR102_u) mean(PR10ag_u)]
[mean(PR1001_u) mean(PR1002_u) mean(PR100ag_u)]
