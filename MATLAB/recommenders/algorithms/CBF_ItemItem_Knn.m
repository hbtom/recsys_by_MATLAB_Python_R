    ...
    methods
         function output = predictRating(obj,userIds_te,itemId_te)
            ...
            item_Id2idx_tr_ob = table2array(obj.item_Id2idx_tr) ;
              ...
              % For each user from test with true itemId (itemId_te), find
              % the corresponding new user Id in train.
              ind_int = ismember(user_Id2idx_tr_ob(:,1),userIds_te);
              new_userIds_tr = [user_Id2idx_tr_ob(ind_int,1) user_Id2idx_tr_ob(ind_int,2)];
        
              % OBS: Note, some users from train may not exist in test.
              % Therefore, the correct user Ids are embedded also in
              % 'new_userIds_tr' to be able to find the non-existing ones!
              ...
              % In this section, we would like to recommend topredict
              % rating of a target user to a target item ('itemId_te').
              for u = 1 : size(new_userIds_tr,1)
                  ...
                  if size(dist_corated_itemIds,1) > nn_ob
                      ...
                      nn_idx  = sim_idx(1:nn_ob);
                      nn_sim_score = sim_score(1:nn_ob);
                           nn_dist = dist_corated_itemIds(nn_idx);
