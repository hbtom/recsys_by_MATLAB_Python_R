function output = evaluate_urms_custom_map_avf(urmTest,urmPred1_str,urmPred_base1_str,urmPred_base2_str,cut_off_trg,flag_rec)
tic
rng(12563);
        ...
        if mod(p,25000)==1
            fprintf('%d true users evaluated \n',p)
              ...
              p = p + 1;
               ...
               map_ranked_list_avf(p) = AP_at_k(ranked_list_avf,pos_items_ids,cut_off_trg);
...
fprintf('Evaluation completed! %d evaluated out of %d \n',p,size(urmTest,1))
       ...
       output.map_ranked_list_avf_all = mean(map_ranked_list_avf)  ;
