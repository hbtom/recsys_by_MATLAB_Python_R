...
featCombList = {'Type3_part_1','Type3_part_2','Type3_part_3','All'};
metric = 'MAP';
rec    = 3;
combCnt = 0 ;
aggrCnt = 0 ;

for combCnt = 1 : length(featCombList)
    for aggrCnt = 1 : length(featAggrList)
        ...
        for fld = 1 : 5
            try
                file = ['Eval_res_RecSys_res_nn_10_feat_AVF_trailers_fps_1_featAggr_' featAggr '_featComb_' featComb '_Norm_2_fld_' num2str(fld) 'of5_cutoff_10_' metric '_rec' num2str(rec) '.mat'];
                load(fullfile(outAddr,'Eval',file));
            catch
                file = ['Eval_res_RecSys_res_nn_10_feat_AVF_trailers_fps1_featAggr_' featAggr '_featComb_' featComb '_Norm_2_fld_' num2str(fld) 'of5_cutoff_10_' metric '_rec' num2str(rec) '.mat'];
                load(fullfile(outAddr,'Eval',file));
            end
            s(fld) = output.map_ranked_list_avf_all;
        end
...
VAL= [VAL1 VAL2];
 ...
 fprintf('\\multirow{4}{*}{\\textbf{Avg}} &color &- &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f \\\\ \n',...
     VAL(1,1),VAL(1,2),VAL(1,3),VAL(1,4),VAL(1,5),VAL(1,6),VAL(1,7),VAL(1,8))
 fprintf(' &object &- &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f \\\\ \n',...
     VAL(2,1),VAL(2,2),VAL(2,3),VAL(2,4),VAL(2,5),VAL(2,6),VAL(2,7),VAL(2,8))
 fprintf(' &texture &- &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f \\\\ \n',...
     VAL(3,1),VAL(3,2),VAL(3,3),VAL(3,4),VAL(3,5),VAL(3,6),VAL(3,7),VAL(3,8))
 fprintf(' &color + object + texture &- &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f \\\\ \\hline \n',...
     VAL(4,1),VAL(4,2),VAL(4,3),VAL(4,4),VAL(4,5),VAL(4,6),VAL(4,7),VAL(4,8))
