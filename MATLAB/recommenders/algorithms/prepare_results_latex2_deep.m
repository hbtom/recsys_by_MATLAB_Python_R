   ...
   for combCnt = 1 : 1 %length(featCombList)
       for aggrCnt = 1 : 1 %length(featAggrList)
           ...
           for fld = 1 : 1
               ...
               s(fld) = output.mrr_ranked_list_avf_all;
...
%             VAL1(combCnt,aggrCnt) = mean(s);
  ...
  VAL = [VAL11 VAL12 VAL13 VAL14 VAL15 VAL16 VAL17 VAL18 VAL19];
  [vmax1,indm1] = max(VAL);
   ...
   for combCnt = 1 : 1 %length(featCombList)
       for aggrCnt = 1 : 1 %length(featAggrList)
           ...
           for fld = 1 : 5
               ...
               %                s(fld) = output.mrr_ranked_list_avf_all;
               sthr19(fld) = output.mrr_ranked_list_hybrid_thr19;
               sthr28(fld) = output.mrr_ranked_list_hybrid_thr28;
               sthr37(fld) = output.mrr_ranked_list_hybrid_thr37;
               sthr46(fld) = output.mrr_ranked_list_hybrid_thr46;
               sthr55(fld) = output.mrr_ranked_list_hybrid_thr55;
               sthr64(fld) = output.mrr_ranked_list_hybrid_thr64;
               sthr73(fld) = output.mrr_ranked_list_hybrid_thr73;
               sthr82(fld) = output.mrr_ranked_list_hybrid_thr82;
               sthr91(fld) = output.mrr_ranked_list_hybrid_thr91;
               
           end
           %            VAL2(combCnt,aggrCnt) = mean(s);
           
           VAL21(combCnt,aggrCnt) = mean(sthr19);
           VAL22(combCnt,aggrCnt) = mean(sthr28);
           VAL23(combCnt,aggrCnt) = mean(sthr37);
           VAL24(combCnt,aggrCnt) = mean(sthr46);
           VAL25(combCnt,aggrCnt) = mean(sthr55);
           VAL26(combCnt,aggrCnt) = mean(sthr64);
           VAL27(combCnt,aggrCnt) = mean(sthr73);
           VAL28(combCnt,aggrCnt) = mean(sthr82);
           VAL29(combCnt,aggrCnt) = mean(sthr91);
  ...
  VAL = [VAL21 VAL22 VAL23 VAL24 VAL25 VAL26 VAL27 VAL28 VAL29];
  [vmax2,indm2] = max(VAL);
  
   fprintf('&visual signal + visual signal  &%s & Late - Bodra (%d,%d) &%0.4f &%0.4f \\\\ \n',...
       hybridFeat,indm1,indm2,vmax1,vmax2)
%    
%    fprintf('\\multirow{4}{*}{\\textbf{Avg}} &genre (jacard) &- &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f &%0.4f \\\\ \n',...
%        VAL(1,1),VAL(1,2),VAL(1,3),VAL(1,4),VAL(1,5),VAL(1,6),VAL(1,7),VAL(1,8))
